\section{A sharp continuous \texorpdfstring{$\PF_4$}{PF4} separator}
\label{sec:separator}
...
\begin{equation}\label{eq:separator-cumulants}
 ...
 q''=-(2c)^4\kappa_4.
\end{equation}
Since the support interval has length six, Popoviciu's inequality yields
\(0\leq\kappa_2\leq9\), and hence
...
\end{equation}
The exact expansion in \appref{app:separator} proves that \(H\) is
...
\[
 \operatorname{disc}(R_c)
 \in -6.0446137251764984704554480068690598
       +[-3.14,3.14]10^{-34},
\]
which is strictly negative. Thus the real cubic has a nonreal conjugate pair.
