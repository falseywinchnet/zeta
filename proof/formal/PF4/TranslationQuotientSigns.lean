...
/-!
# First and second translation-quotient signs

This module derives the first strict quotient sign from the literal kernel
curvature and factors the second quotient derivative through the exact
lower-order `Lambda` object used by the S05 quotient reduction.

The analytic inputs `kernelCurvature > 0` and `lowerLambda > 0` remain named
premises. No quotient derivative, Wronskian, finite difference, integral, or
minor sign is assumed.
-/

namespace PF4.TranslationQuotientSigns

open PF4.TranslationQuotientTower

/-- The logarithmic slope, defined directly from the kernel jet. -/
noncomputable def logSlope (Φ Φ1 : ℝ → ℝ) : ℝ → ℝ :=
  fun t => Φ1 t / Φ t

/-- The exact curvature boundary in the kernel jet. -/
...
/-- The three-point lower-order `Lambda` from the S05 logarithmic-slope
reduction. Its later coordinate realization is a separate instance bridge. -/
...
positive kernel ratio and an oriented logarithmic-slope chord. -/
...
/-- The first quotient sign follows globally from ordered columns, positive
`Φ`, and positive exact curvature. -/
...
/-- Exact first-level logarithmic-rate factorization. Derivative uniqueness
connects the differentiated product factorization to the maintained
...
/-- The direct rate difference is exactly the S05 order-three rate. -/
...
/-- Exact maintained second-quotient derivative factorization through the S05
lower-order `Lambda`. -/
...
theorem secondQuotD_pos_of_lowerLambda_pos
  ...
  have hanti := logSlope_strictAnti_of_kernelCurvature_pos hΦ hΦ1 hΦpos hqpos
  have hfirstC := firstQuotD_pos_of_kernelCurvature_pos hΦ hΦ1 hΦpos hqpos hac
  have hfirstB :=
    firstQuotD_pos_of_kernelCurvature_pos hΦ hΦ1 hΦpos hqpos (hac.trans hcb)
  have hAb : ∀ s,
...
end PF4.TranslationQuotientSigns
