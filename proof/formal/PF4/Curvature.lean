...
private theorem hasDerivAt_increasingPrimitive
  ...
  have hx : HasDerivAt (fun x : ℝ => x - p) 1 t := by
    simpa only [id_eq] using (hasDerivAt_id t).sub_const p
...
private theorem hasDerivAt_decreasingPrimitive
  ...
  have hx : HasDerivAt (fun x : ℝ => z - x) (-1) t := by
    simpa only [id_eq] using (hasDerivAt_id t).const_sub z
...
theorem increasing_integral
    ...
    (hint : IntervalIntegrable (fun t => (t - p) * curvature Q2 t) volume p z) :
    (∫ t in p..z, (t - p) * curvature Q2 t) =
      (z - p) ^ 2 - (z - p) * Q1 z + Q z - Q p := by
  rw [intervalIntegral.integral_eq_sub_of_hasDerivAt
    (fun t ht => hasDerivAt_increasingPrimitive
      (hQ t (by simpa [uIcc_of_le hpz] using ht))
      (hQ1 t (by simpa [uIcc_of_le hpz] using ht))) hint]
...
theorem decreasing_integral
    ...
    (hQ : ∀ t ∈ Icc p z, HasDerivAt Q (Q1 t) t)
    (hQ1 : ∀ t ∈ Icc p z, HasDerivAt Q1 (Q2 t) t)
    (hint : IntervalIntegrable (fun t => (z - t) * curvature Q2 t) volume p z) :
    (∫ t in p..z, (z - t) * curvature Q2 t) =
      (z - p) ^ 2 + (z - p) * Q1 p + Q p - Q z := by
  rw [intervalIntegral.integral_eq_sub_of_hasDerivAt
    (fun t ht => hasDerivAt_decreasingPrimitive
      (hQ t (by simpa [uIcc_of_le hpz] using ht))
      (hQ1 t (by simpa [uIcc_of_le hpz] using ht))) hint]
...
theorem hasDerivAt_coordinateLambda_left
  ...
  have hc : HasDerivAt (fun _ : ℝ => Q z) 0 p := hasDerivAt_const p (Q z)
  have hnum := hc.sub hQ
  have hden := (hasDerivAt_id p).const_sub z
  have hquot := hnum.div hden (sub_ne_zero.mpr hpz.ne')
  have hmain := ((hasDerivAt_id p).const_sub w).add hquot |>.sub_const
    ((Q w - Q z) / (w - z))
  have hfun : (fun x => coordinateLambda Q x z w) =
      (fun x : ℝ => (w - x) + (Q z - Q x) / (z - x) -
        (Q w - Q z) / (w - z)) := by
    funext x
    rfl
  have hval : -coordinateDelta Q Q1 p z =
      -1 + ((-Q1 p) * (z - p) - (Q z - Q p) * (-1)) / (z - p) ^ 2 := by
    unfold coordinateDelta chordSlope
    field_simp [sub_ne_zero.mpr hpz.ne']
    ring
  have heq : Filter.EventuallyEq (nhds p) (fun x => coordinateLambda Q x z w)
      (fun x : ℝ => (w - x) + (Q z - Q x) / (z - x) -
        (Q w - Q z) / (w - z)) := Filter.Eventually.of_forall fun x => by
    exact congrFun hfun x
  apply (hmain.congr_of_eventuallyEq heq).congr_deriv
  simpa [id_eq] using hval.symm
  ...
  have hraw : 0 < ∫ t in p..z, (z - t) * curvature Q2 t :=
    intervalIntegral.integral_pos hpz hcont
      (fun t ht => mul_nonneg (sub_nonneg.mpr ht.2)
        (hκpos t ⟨le_of_lt ht.1, ht.2⟩).le)
      ⟨(p + z) / 2, by constructor <;> linarith,
        mul_pos (by linarith)
          (hκpos ((p + z) / 2) (by constructor <;> linarith))⟩
  rw [coordinateDelta_eq_triangular hpz hQ hQ1 hκcont]
...
theorem coordinateLambda_pos
  ...
  have hl : 0 < ∫ t in p..z, (t - p) * curvature Q2 t :=
    intervalIntegral.integral_pos hpz hlcont
      (fun t ht => mul_nonneg (sub_nonneg.mpr ht.1.le)
        (hκpos t ⟨ht.1.le, ht.2.trans hzw.le⟩).le)
      ⟨(p + z) / 2, by constructor <;> linarith,
        mul_pos (by linarith)
          (hκpos ((p + z) / 2) (by constructor <;> linarith))⟩
  have hr : 0 < ∫ t in z..w, (w - t) * curvature Q2 t :=
    intervalIntegral.integral_pos hzw hrcont
      (fun t ht => mul_nonneg (sub_nonneg.mpr ht.2)
        (hκpos t ⟨hpz.le.trans ht.1.le, ht.2⟩).le)
      ⟨(z + w) / 2, by constructor <;> linarith,
        mul_pos (by linarith)
          (hκpos ((z + w) / 2) (by constructor <;> linarith))⟩
  rw [coordinateLambda_eq_triangular hpz hzw hQ hQ1 hκcont]
