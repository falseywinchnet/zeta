...
theorem derivative_eq_of_eqOn_Ici
  ...
  exact (uniqueDiffOn_Ici 0 t ht).eq_deriv (Set.Ici 0) hf'
    hg.hasDerivWithinAt
...
theorem hasDerivAt_thetaSeriesJet_of_lt_six
  ...
  rcases IntervalControl.derivativeTowerThroughSix_at_nonneg ht with
    ⟨h0, h1, h2, h3, h4, h5⟩
  have hjCases : j = 0 ∨ j = 1 ∨ j = 2 ∨ j = 3 ∨ j = 4 ∨ j = 5 := by
    omega
  rcases hjCases with h | h | h | h | h | h
  · subst j
    simpa using h0
  · subst j
    simpa using h1
    ...
    calc
      iteratedDeriv j globalRiemannKernel t =
        ...
        simpa only [neg_neg] using
          iteratedDeriv_globalRiemannKernel_neg j (-t)
      _ = (-1 : ℝ) ^ j * thetaSeriesJet j (-t) := by
        rw [iteratedDeriv_globalRiemannKernel_eq_thetaSeriesJet hj
          (neg_nonneg.mpr ht')]
...
theorem thetaMode_pos_of_nonneg (k : ℕ) {t : ℝ} (ht : 0 ≤ t) :
  ...
  rw [hfactor]
  have hbracket :
      0 < 2 * (Real.pi * modeN k ^ 2 * Real.exp (2 * t)) - 3 := by
    nlinarith
  positivity
...
theorem clearedF2_kernelJet_pos_of_nonneg
  ...
  by_cases ht : 0 ≤ t
  · simpa [ht] using hs
  · norm_num [ht, clearedF2_alternating_parity] at ⊢
    exact hs
...
theorem clearedC4_kernelJet_pos_of_nonneg
  ...
  by_cases ht : 0 ≤ t
  · simpa [ht] using hs
  · norm_num [ht, clearedC4_alternating_parity] at ⊢
    exact hs
