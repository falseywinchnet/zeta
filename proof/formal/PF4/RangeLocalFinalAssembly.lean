...
Maintained range-local layer.  The terminal sign theorem below consumes calculus and
...
namespace PF4.RangeLocalFinalAssembly
...
theorem coordinateJetAndSigns_on_kernelCoordinate_Icc
  ...
  intro y hy
  have hyrange := kernelCoordinate_Icc_subset_range hS hqpos hy
...
theorem coordinateQ4_continuousAt_kernelCoordinate
  ...
  have hqAt : ContinuousAt q
      (coordinateInverse S (kernelCoordinate S u)) := by
    simpa [hinvValue] using (hq u).continuousAt
  have hq1At : ContinuousAt q1
      (coordinateInverse S (kernelCoordinate S u)) := by
    simpa [hinvValue] using (hq1 u).continuousAt
  have hq2At : ContinuousAt q2
      (coordinateInverse S (kernelCoordinate S u)) := by
    simpa [hinvValue] using (hq2 u).continuousAt
  have hq3At : ContinuousAt q3
      (coordinateInverse S (kernelCoordinate S u)) := by
    simpa [hinvValue] using (hq3 u).continuousAt
  have hqcomp : ContinuousAt (fun y => q (coordinateInverse S y))
      (kernelCoordinate S u) := ContinuousAt.comp hqAt hinv
  have hq1comp : ContinuousAt (fun y => q1 (coordinateInverse S y))
      (kernelCoordinate S u) := ContinuousAt.comp hq1At hinv
  have hq2comp : ContinuousAt (fun y => q2 (coordinateInverse S y))
      (kernelCoordinate S u) := ContinuousAt.comp hq2At hinv
  have hq3comp : ContinuousAt (fun y => q3 (coordinateInverse S y))
      (kernelCoordinate S u) := ContinuousAt.comp hq3At hinv
...
theorem coordinateQ4_continuousOn_kernelCoordinate_Icc
  ...
  intro y hy
  rcases kernelCoordinate_Icc_subset_range hS hqpos hy with ⟨u, rfl⟩
...
end PF4.RangeLocalFinalAssembly
