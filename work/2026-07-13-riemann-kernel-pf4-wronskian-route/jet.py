...
def cumulants(derivatives: list[arb], highest: int = 8) -> list[arb]:
    """kappa_1..kappa_highest of log Phi via the moment-cumulant recursion.

    kappa_n = r_n - sum_{k=1}^{n-1} C(n-1, k-1) kappa_k r_{n-k}, r_j = f_j/f_0.
    The recursion is validated against the audited hardcoded expansions of
    invariant_jet in the certifier startup self-check.
    """
    from math import comb

    f0 = derivatives[0]
    r = [None] + [derivatives[j] / f0 for j in range(1, highest + 1)]
    kappa: list[arb] = [None, r[1]]
    for n in range(2, highest + 1):
        value = r[n]
        for k in range(1, n):
            value -= comb(n - 1, k - 1) * kappa[k] * r[n - k]
        kappa.append(value)
    return kappa


def invariant_jet(derivatives: list[arb]) -> tuple[arb, arb, arb, arb, arb, arb, arb]:
    f0, f1, f2, f3, f4, f5, f6 = derivatives[:7]
    r1 = f1 / f0
    r2 = f2 / f0
    r3 = f3 / f0
    r4 = f4 / f0
